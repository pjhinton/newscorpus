...
class DataFeed(Base):
    ...
    def __repr__(self):
        return '<DataFeed id="{0}" feed_name="{1}">'.format(
            self.id,
            self.feed_name
        )
...
class FeedItem(Base):
    ...
    article_retrievals = relationship(
        'ArticleRetrieval',
...
class ArticleRetrieval(Base):
    ...
    feed_item = relationship(
        'FeedItem',
        back_populates='article_retrievals'
    )
